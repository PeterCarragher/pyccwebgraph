...
✅ **Discover related domains** via backlinks or outlinks  
✅ **Convert to NetworkX, NetworKit, or igraph** for analysis  
✅ **Run network algorithms** (PageRank, community detection, centrality)  
✅ **Work with billion-edge graphs** in Python efficiently  
...
## 🎯 When to Use pyccwebgraph

### Use pyccwebgraph when you want to:

✅ **Discover domain networks** interactively in Python/Jupyter  
✅ **Extract subgraphs** for analysis (e.g., all domains linking to a seed set)  
✅ **Convert webgraph data** to NetworkX/NetworKit/igraph  
✅ **Iterate quickly** on discovery queries without writing Java  

### Use cc-webgraph tools directly when you need to:

❌ **Compute global metrics** over the full 93M node graph (e.g., PageRank on entire web)  
❌ **Maximum performance** for batch processing millions of queries  
❌ **Host-level graphs** (100B+ edges, requires more memory)  
❌ **Advanced WebGraph features** (compression, specific graph algorithms)  

**Rule of thumb:** If your workflow is "discover a subgraph → analyze in Python", use pyccwebgraph. If you're computing statistics over the entire graph or need maximum performance, use cc-webgraph's Java tools directly.
...
results = webgraph.discover_backlinks(
    ...
    min_connections=10  # Must link to ≥10 of the seeds
)
...
## 📦 Working with Different Webgraph Versions
